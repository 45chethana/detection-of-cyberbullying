...
from .models import Social,info,contact
...
def contact(request):
     if request.method=="POST":
        ...
        c=contact.objects.create(username=un,email=em,pno=pno,text=msg)
        c.save()
    ...
    if request.method=="POST":
        ...
        if p1==p2:
            if User.objects.filter(username=un).exists():
                ...
                user=User.objects.create_user(first_name=fn,last_name=ln,username=un,password=p2,email=em)
                user.save()
...
def cyberbullying(request):
    if request.method=="POST":
        ...
        data=pd.read_csv(r'C:\Users\Jagan\Desktop\python\django\fraud\fraud\static\dataset\Cyberbullying.csv')
        ...
        data_1.hist(ax=axis)
        plt.show()
        new=info.objects.create(text=cyber,output=predicted_data)
        new.save()
        #cyberdata=CyberTweets.objects.create(tweets=cyber,cyber_data=predicted_data)
        #cyberdata.save()

        return render(request,"prediction.html",{"cyber":predicted_data})
    else:
        return render(request,"cyberbullying.html")
    return render(request,"cyberbullying.html")
...
def social_msg(request):
    social1=Social.objects.all()
    return render(request,"social.html",{"social":social1})
